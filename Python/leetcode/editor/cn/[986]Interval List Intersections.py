...
class Solution:
    def intervalIntersection(self, firstList: List[List[int]], secondList: List[List[int]]) -> List[List[int]]:
        ...
        while first < len(firstList) and second < len(secondList):
            f_item = firstList[first]
            s_item = secondList[second]
            max_left = max(f_item[0], s_item[0])
            min_right = min(f_item[1], s_item[1])
            if max_left <= min_right:
                rst.append([max_left, min_right])
            if min_right == f_item[1]:
                first += 1
            if min_right == s_item[1]:
                second += 1
        return rst
